  ...
  return {
    ...
    configureServer(server) {
      ...
      options.schemas.forEach(({ name, path }) => {
        server.middlewares.use(`${PREFIX}/${name}`, (req, res, next) => {
        ...
        switch (req.method.toLowerCase()) {
          ...
          case 'put': {
            ...
            if (moduleFiles.length < value.length) {
              ...
              // update module
              await Promise.all(
                moduleFiles.map(async (fileName, i) => {
                  const oldModuleName = fileName.split('.')[0];
                  const schema = value.find(
                    moduleSchema => moduleSchema.metadata.name === oldModuleName
                  );
                  ...
                  if (schema) {
                    ...
                    const newName = names.find(name => !oldModuleNames.includes(name));
